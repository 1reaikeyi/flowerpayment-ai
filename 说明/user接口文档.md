...
**响应**: `Result<Void>` → 仅返回 `code=200`，**data 为 null**

> 注意：当前代码中服务端已生成 Token 并写入 Redis，但控制器使用 `Result.success()` 返回，**响应体未携带 Token**（疑似 Bug，前端暂无法从响应体获取 Token）。
...
> 注意：新增（POST）与修改（PUT）接口的参数**未加 `@RequestBody`**，需使用 `form-data` / `x-www-form-urlencoded` / Query 方式传参，不能传 JSON。
...
> 注意：该接口语义对应状态 1（用户下单 ORDER），但**当前代码实际将状态更新为 COOKING（3）**，疑似 Bug。
...
> 注意：该接口语义对应状态 2（用户确认支付 PAYMENT），但**当前代码实际将状态更新为 COOKING（3）**，疑似 Bug。实际支付跳转/回调由 `/pay/**` 接口承担（不在本文档范围）。
